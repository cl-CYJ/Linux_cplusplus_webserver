...
//网站根目录
const char* doc_root = "/home/cyj/linux/Linux_cplusplus_webserver/resources";
...
//修改epoll中的文件描述符，重置socket上的EPOLLONESHOT事件，以确保下一次可读时，EPOLLIN事件能够触发
...
void http_conn::close_conn() {
    if (m_sockfd != -1) {
        ...
        m_sockfd = -1;
        m_user_count --; //关闭一个连接，客户数量减一
    }
}

//一次性读完数据，循环读取数据，直到无数据或对方关闭连接
bool http_conn::read() {
...
//解析请求头，解析请求头与空行使用同一个函数，根据开头是否是'\0'来判断
